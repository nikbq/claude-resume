#!/usr/bin/env python3
"""
Standalone Claude Resume Runner with improvements
Run directly with: curl -s https://raw.githubusercontent.com/nikbq/claude-resume/main/run.py | python3
"""
...
class ChatHistoryHandler(SimpleHTTPRequestHandler):
    ...
    def serve_html(self):
        ...
        is_home = Path(current_dir).resolve() == Path.home()
        title = "Claude Chat History Viewer" if is_home else f"Claude Chats - {Path(current_dir).name}"
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .context-info {{ background: #e3f2fd; padding: 10px 15px; border-radius: 6px; margin-bottom: 20px; font-size: 14px; color: #1976d2; }}
...
<body>
    <div class="container">
        <h1>{title}</h1>
        {"" if is_home else f'<div class="context-info">Showing chats from: {current_dir}</div>'}
            ...
            listDiv.innerHTML = filteredChats.map(chat => `
                <div class="chat-item" onclick="showChatDetails('${{chat.id}}')">
                    <div class="chat-header">
                        <div>
                            ${{chat.summary ? 
                                '<div class="chat-heading">' + escapeHtml(chat.summary) + '</div>' : 
        ...
        function showChatDetails(chatId) {{
            ...
            modalBody.innerHTML = `
                <div style="margin-bottom: 20px; padding: 15px; background: #f5f5f5; border-radius: 6px;">
                    ...
                    ${{chat.summary ? '<br><strong>Summary:</strong> ' + escapeHtml(chat.summary) : ''}}
                </div>
...
def main():
    ...
    # Show context info
    current_dir = os.getcwd()
    if Path(current_dir).resolve() == Path.home():
        print("Running from home directory - showing all projects")
    else:
        print(f"Running from {current_dir} - showing related chats only")
